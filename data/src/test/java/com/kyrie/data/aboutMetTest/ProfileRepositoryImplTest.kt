...
class ProfileRepositoryImplTest {
    ...
    @Test
    fun `getProfile should emit success when Firebase returns data`() = runTest {
    ...
    @Test
    fun `getProfile should emit error when Firebase fails`() = runTest {
        val errorMessage = "Firebase error"
        coEvery { mockFireStore.collection(any()).document(any()).get().await() } throws Exception(
            errorMessage
        )
        val result = repository.getProfile()
